...
function open_mailbox($auth_user, $accountid)
{
  ...
  // connect to the POP3, POP3/ssl, IMAP/ssl or IMAP server the user has selected
  ...
  if($settings[type]=='POP3')
    $mailbox .= '/pop3}INBOX';
  else if ($settings[type]=='POP3SSL')
    $mailbox .= '/pop3/ssl/novalidate-cert}INBOX';
  else if ($settings[type]=='IMAPSSL')
    $mailbox .= '/ssl/novalidate-cert}INBOX';
  else if ($settings[type]=='IMAP')
  $mailbox .= '}INBOX';

  // suppress warning, remember to check return value  
  echo 'Attempting to open mailbox: <br>';
  echo $mailbox . '<br><br>';
